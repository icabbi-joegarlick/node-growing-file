...
GrowingFile.open = function(path, options) {
  ...
  options = options || {};
  ["timeout", "interval"].forEach(function(option) {
  ...
  if (options.startFromEnd) file._getFileSizeAndReadUntilEof();
  else file._readUntilEof();
...
GrowingFile.prototype._readUntilEof = function() {
  if (this._paused || this._reading || this._dontend) {
    return;
  }
...
GrowingFile.prototype.close = function() {
  this._stream.unpipe();
  this.destroy();
  this.emit("end");
};
